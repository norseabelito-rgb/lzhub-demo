...
async function main() {
  ...
  // ==========================================================================
  // 10. ONBOARDING CONFIG (Manager-customizable content)
  // ==========================================================================
  console.log('Creating onboarding config...')

  // Helper to wrap plain text paragraphs in <p> tags for Tiptap compatibility
  function textToHtml(text: string): string {
    return text
      ...
      .map((block) => {
        ...
        if (!trimmed) return ''
        // Wrap each line within a block in <p> tags
        return trimmed
          .split('\n')
  ...
  const ndaHtml = textToHtml(`ACORD DE CONFIDENTIALITATE SI NEDIVULGARE
  ...
  const docContents = [
    {
      ...
      content: `REGULAMENT INTERN LASERZONE ARENA
...
5.3 Participarea la instructajele periodice de securitate este obligatorie.`,
    },
    {
      ...
      minReadingSeconds: 30,
      content: `PROCEDURI DE SIGURANTA LASERZONE ARENA
...
5.3 Spalati mainile frecvent, mai ales dupa manipularea echipamentelor.`,
    },
    {
      ...
      minReadingSeconds: 30,
      content: `GHID UTILIZARE ECHIPAMENT LASERZONE
...
5.5 Stingere lumini, incuiere.`,
  ...
  for (let i = 0; i < docContents.length; i++) {
    await prisma.onboardingDocument.create({
      data: {
        configId: 'default',
        title: docContents[i].title,
        content: textToHtml(docContents[i].content),
        minReadingSeconds: docContents[i].minReadingSeconds,
        sortOrder: i,
      },
    })
  }
  ...
  // Seed video chapters
  const chapters = [
    { title: 'Introducere si prezentare generala', timestamp: 0, sortOrder: 0 },
    { title: 'Proceduri de siguranta obligatorii', timestamp: 45, sortOrder: 1 },
    { title: 'Operarea echipamentului laser', timestamp: 120, sortOrder: 2 },
    { title: 'Interactiunea cu clientii', timestamp: 200, sortOrder: 3 },
    { title: 'Proceduri de urgenta', timestamp: 280, sortOrder: 4 },
    { title: 'Curatenie si inchidere', timestamp: 350, sortOrder: 5 },
  ]

  for (const ch of chapters) {
    await prisma.onboardingVideoChapter.create({
      data: {
        configId: 'default',
        title: ch.title,
  ...
  for (const q of quizQuestions) {
    await prisma.onboardingQuizQuestion.create({
      data: {
        configId: 'default',
        type: q.type,
        text: q.text,
  ...
  console.log('  Created onboarding config with documents, chapters, and quiz questions')
